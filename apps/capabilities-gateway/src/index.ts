...
fastify.addHook('preHandler', async (request, reply) => {
  ...
  if (serviceKey) {
    ...
    if (rawKey === serviceKey) {
      ...
      (request as typeof request & RequestWithTenant).plan = 'capabilities-enterprise';
  ...
  const quota = await checkCapQuota(keyInfo.tenantId, currentMonthKey());
  const isApplyRequest = request.url.includes('/apply');
  if ((isApplyRequest && !quota.appliesOk) || (!isApplyRequest && !quota.plansOk)) {
...
fastify.get('/health', async (_request, reply) => {
  const upstream = await checkUpstreamHealth(env.CAPABILITIES_INTERNAL_URL);
  reply.send({
    status: 'healthy',
    upstream,
...
async function proxyRequest(request: import('fastify').FastifyRequest, reply: import('fastify').FastifyReply) {
  ...
  const targetUrl = `${env.CAPABILITIES_INTERNAL_URL}${targetPath}`;
  ...
  try {
    ...
    if (response.status < 500 && !env.DEV_BYPASS_AUTH && !(request as typeof request & RequestWithTenant).isServiceKey) {
      const type = targetPath.includes('/apply') ? 'apply' : 'plan';
      Promise.resolve(incrementCapUsage(tenantId, currentMonthKey(), type)).catch(console.error);
    }
    ...
    request.log.error({ err: error }, 'capabilities gateway upstream request failed');
    return sendError(reply, 502, 'Failed to reach upstream service', 'UPSTREAM_ERROR', request.id);
  }
}

fastify.setErrorHandler((error, request, reply) => {
  request.log.error({ err: error }, 'capabilities gateway unhandled error');
...
try {
  ...
  fastify.log.info(`Capabilities gateway listening on ${env.HOST}:${env.PORT}`);
} catch (error) {
