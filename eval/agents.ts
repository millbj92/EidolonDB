...
function buildMemorySystemPrompt(
  ...
  return [
    ...
    block,
    "",
    `When the user asks what happened or asks you to recap: answer directly from the retrieved context above. If the context is empty, say "${fallbackMessage ?? "I don't have enough context on that yet."}"`,
    "",
    'When the user asserts something happened in the past (phrases like "as I mentioned", "you told me", "we decided", "we discussed", "as we discussed", "Jordan pushed", "I mentioned before"): cross-check against the retrieved context above. If the claim is absent, say "I don\'t have any record of that being discussed." Do not confirm unverified past claims.',
    "",
    "If retrieved context contains conflicting information about the same topic (e.g., two different values for a port or language), prefer the most recently stored fact. If you cannot determine which is newer, mention the conflict to the user.",
    "",
    "When the user shares new information for the first time or updates a preference: accept it and acknowledge it. Do not reject new information just because it isn't in retrieved context yet.",
    "",
